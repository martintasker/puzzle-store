...
const INITIAL_STATE = Immutable.Map({puns: Immutable.List(), progress: Immutable.List()});
...
describe('puzzleProgress status tests', function() {
  it('updates status', function() {
    ...
    expect(state.get('puns')).toEqual(Immutable.fromJS([{index: 0, status: 'wrong'},{index: 1, status: 'not-done'}]));
